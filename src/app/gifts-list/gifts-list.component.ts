...
export class GiftsListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParamMap.subscribe(paramMap => {
      ...
      // tslint:disable-next-line:max-line-length
      this.selectedSorting = paramMap.get('sort')?.length > 0 && this.allowedSort.includes(paramMap.get('sort')) ? SortingModel[paramMap.get('sort')] : SortingModel[SortingModel.Latest];
  ...
  prepareRouterParams(): object {
    ...
    // tslint:disable-next-line:max-line-length
    if (SortingModel[this.selectedSorting] !== SortingModel[SortingModel.Latest]) { queryParamsObj['sort'] = SortingModel[this.selectedSorting].toString(); }
